...
\begin{frame}{SQL Commands Reference - DQL}
   ...
   \begin{tabularx}{\textwidth}{|l|l|X|}
     ...
     \textbf{Command} & \textbf{Applies to} & \textbf{Description} \\
      ...
      \hyperref[frm:groupby]{SUM} & Rows & Sum values (aggregate) \\
      \hyperref[frm:groupby]{AVG} & Rows & Average values (aggregate) \\
      \hyperref[frm:groupby]{MIN} & Rows & Minimum value (aggregate) \\
      \hyperref[frm:groupby]{MAX} & Rows & Maximum value (aggregate) \\
...
GROUP BY col1, col2;
      ...
      \textbf{Description:}
      \begin{itemize}
         \item Groups rows that share the same values for the grouped column(s)
         \item Use with aggregate functions: COUNT, SUM, AVG, MIN, MAX
         \item Useful to compute summaries per category (e.g., per customer)
      \end{itemize}
   \end{columns}
\end{frame}

\begin{frame}[fragile]{COUNT and Aliases - Label Aggregates}
\label{frm:aliases}
   \begin{columns}[T]
      \column{0.45\textwidth}
      \textbf{Syntax / Examples:}
      \begin{lstlisting}[numbers=none]
-- Count rows per category and name the column
...
GROUP BY user_id;
      ...
      \textbf{Notes:}
      \begin{itemize}
         \item `AS` gives a readable name to aggregate output columns
         \item Aggregates ignore NULLs (except COUNT(*))
         \item Use aliases in ORDER BY or in outer queries for clarity
      \end{itemize}
   \end{columns}
\end{frame}
